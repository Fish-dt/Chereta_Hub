  ...
  try {
    ...
    for (const auction of endedAuctions) {
      ...
      if (winningBid) {
        ...
        const reserveMet = !auction.reservePrice || winningBid.bidAmount >= auction.reservePrice
        ...
        if (reserveMet) {
          ...
          await db.collection("notifications").insertOne({
            type: "auction_ended",
            title: "Your auction has ended",
            message: `Your auction for "${auction.title}" has ended but the reserve price of $${auction.reservePrice} was not met. Highest bid was $${winningBid.bidAmount}.`,
            recipientId: auction.sellerId,
            auctionId: auction._id,
            isRead: false,
            createdAt: new Date(),
          ...
          await db.collection("notifications").insertOne({
            ...
            message: `The auction for "${auction.title}" has ended but the reserve price was not met. Your bid of $${winningBid.bidAmount} was the highest.`,
            recipientId: winningBid.bidderId,
            auctionId: auction._id,
            isRead: false,
            createdAt: new Date(),
          ...
          console.log(`❌ Auction "${auction.title}" ended - reserve price not met`)
        }
      } else {
        // No bids received
        await db.collection("auctions").updateOne(
